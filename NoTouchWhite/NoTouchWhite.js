...
function speedup() 
{
        ...
        {
                tip_list = ["Great!", "Unbeleviable!", "Perfect!", "Good Work!"]
                index = Math.floor(Math.random * tip_list.length);
                alert(tip_list[index]);
        }
}
...
function over() 
{
        ...
        if ((rows.length == 5) && (rows[rows.length - 1].pass !== 1)) //最后一列黑块没有点过
        {
            fail();
        }
...
function fail() 
{
        clearInterval(clock);   //停止定时触发
        flag = false;
        confirm('你的最终得分为 ' + parseInt($('score').innerHTML)); 
        var con = $('con');
        con.innerHTML =  "";    //清空
        $('score').innerHTML = 0;
        con.style.top = '-408px';       //隐藏
        speed = const_speed;
}
